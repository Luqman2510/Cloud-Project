    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <form method="POST" action="{{ route('events.update', $event) }}" x-data="{
                location: null,
                placeId: @js($event->place_id),
                places: @js($event->location->places),
                ...
                <div class="grid gap-6 mb-6 md:grid-cols-2">
                    ...
                    <div>
                        ...
                        <select id="location_id" x-on:change="onLocationChange" name="location_id"
                            class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 
                            focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 
                            dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                            <option>Choose a location</option>
                            @foreach ($locations as $location)
                                <option value="{{ $location->id }}" @selected($location->id === $event->location_id)>{{ $location->name }}
                                </option>
                            @endforeach
                        </select>
                        @error('location_id')
                    ...
                    <div>
                        ...
                        <select id="place_id" name="place_id"
                            ...
                            <template x-for="place in places" :key="place.id">
                                <option x-bind:value="place.id" x-text="place.name" :selected="place.id === placeId">
                                </option>
                            </template>
                        </select>
                        @error('place_id')
                    ...
                    <div>
                        ...
                        <input type="number" id="num_attendees" name="num_attendees"
                            class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 f
                            ocus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 
                            dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                            value="{{ old('num_attendees', $event->num_attendees) }}">
                        @error('num_attendees')
                            <div class="text-sm text-red-400">{{ $message }}</div>
                        @enderror
                    </div>
                    <div>
                        ...
                        <textarea id="description" name="description" rows="4"
                            ...
                            dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">{{ $event->description }}</textarea>
                        @error('description')
                            <div class="text-sm text-red-400">{{ $message }}</div>
                        @enderror
                    </div>
